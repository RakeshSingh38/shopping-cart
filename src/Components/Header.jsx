...
function Header() {
    ...
    const navHandle = () => setToggle(!toggle);
    const toogleFalse = () => setToggle(false);
    const createAccount = () => {
    ...
    return (
        <>
            <div className="px-[10px] py-4 sm:py-[20px] bg-[#FFFFFF] flex justify-between items-center fixed w-full z-50 top-0 shadow-md">
                <i
                ...
                <ul
                    className={`  text-[1.1rem]  itms-center leading-none tracking-wide absolute  left-0 flex-col max-w-[425px] w-full bg-white text-xl h-screen sm:justify-center sm:static sm:flex-row sm:bg-transparent z-[999] sm:h-fit sm:w-fit sm:flex ${
                        toggle
                            ? "flex active-nav top-0 gap-3"
                            : "hidden top-[70px] gap-[0px]"
                    }`}
                >
                    {toggle ? (
                        <div className="flex justify-between items-center bg-black text-white px-4 py-5">
                            ...
                            <li>
                                <i
                                    className="ri-close-fill text-[1.5rem] cursor-pointer"
                                    onClick={navHandle}
                                ></i>
                            </li>
                        </div>
            ...
            <li>Services</li>
                    ...
                    {toggle ? (
                        <div className="px-4 py-1">
                            <button
                                className="uppercase w-full bg-black outline-none text-white py-4 px-4 rounded-full mt-6 text-sm "
                                onClick={sigin}
                            >
                                SIGN IN
                            </button>
                            <p
                                className="text-center mt-8 uppercase text-[0.9rem] text-black"
                ...
                <div className="flex gap-[5px] justify-around items-center text-[1.3rem] sm:gap-[20px]">
                    ...
                    <NavLink to="/shoppingCart">
                        <div className="relative">
                            ...
                            <div className="absolute top-0 right-0 w-[15px] h-[15px] bg-[#9C6D3A] flex justify-center items-center text-white rounded-full text-[10px] ">
                                {cartItems.length}
                            </div>
                        </div>
                    </NavLink>
